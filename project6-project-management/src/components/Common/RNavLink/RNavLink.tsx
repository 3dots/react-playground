...
export function RNavLink(props: IRNavLinkProps) {
  ...
  let className = "underline disabled:bg-stone-400 disabled:cursor-not-allowed hover:text-amber-900";
  if (classNameProp) className += ` ${classNameProp}`;

  let activeClass = "text-amber-700";

  return (
    <NavLink className={(p) => `${className} ${p.isActive ? activeClass : ""}`} {...rest}>
      {children}
    </NavLink>
  );
}