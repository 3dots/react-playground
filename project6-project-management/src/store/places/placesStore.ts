...
import { asyncTryCatchLoadingWrapper, asyncTryCatchWrapper, tryCatchWrapper } from "../app/appStore";
...
export const usePlacesStore = create<IPlacesStateWrapper>((set, get) => ({
  ...
  getPlaces: asyncTryCatchWrapper(async () => {
    const [places, selectedPlaces] = await Promise.all([placesApi.getPlaces(), placesApi.getSelectedPlaces()]);
    set((sw) => ({ state: sw.state.setAvailablePlaces(places, selectedPlaces) }));
  }),
  setLocation: tryCatchWrapper((latitude: number | null, longitude: number | null) => {
  ...
  selectPlace: asyncTryCatchLoadingWrapper(async (place: IPlaceDto) => {
    ...
    await placesApi.saveSelectedPlaces(get().state.selectedPlaces.map(x => x.id));
  }),
