...
export function RButton(props: PropsWithChildren<IRButtonProps>) {
  ...
  let className =
    "py-2 px-3 rounded-md hover:bg-amber-900 focus:bg-amber-900 flex disabled:bg-stone-400 disabled:cursor-not-allowed";
  switch (buttonType) {
    case EnButtonType.Primary:
      className += ` text-gray-400 ${isDarkBackground ? "bg-stone-700" : "bg-stone-900"}`;
      break;
    case EnButtonType.Secondary:
      className += ` hover:text-white focus:text-white disabled:hover:text-inherit`;
      break;
  }
